...
/mob/proc/CtrlClickOn(var/atom/A)
	if(isturf(A) || isobj(A)) // Used as hotkey for bolt action and pumping shotgun and makes that we can still pull mobs.
		var/obj/item/weapon/gun/projectile/ba_rifle/O = src.get_active_hand()
		var/obj/item/weapon/gun/projectile/smle/ENF = src.get_active_hand()//Yep, need two procs due to the difference in loading methods
		var/obj/item/weapon/gun/projectile/shotgun/pump/SHO = src.get_active_hand()
		if(istype(O))
			O.bolt_action(src)
			return

		if(istype(ENF))
			ENF.smle_bolt_action(src)
			return
...
/atom/MouseDown(location,control,params)
	...
	if(modifiers["left"]) // Without this check - middle button will be used too, and we don't want that.
		var/obj/item/weapon/gun/projectile/automatic/W = usr.get_active_hand()
		...
		if((usr.a_intent == "harm") && istype(W) && isturf(usr.loc) && !(W == src) && !usr.client.buildmode && !usr.restrained() && !usr.in_throw_mode && (W.fire_delay > 0) && !istype(src, /obj/screen))
